...
def user_settings():
    if request.method == "POST":
        ...
        if new_password:
            current_user.set_password(new_password)  # If using hashed passwords
    ...
    return render_template("user_settings.html", machines=[], current_user=current_user)
...
def subuser_dashboard():
    ...
    # ✅ Check if lube was done anytime this week
    lube_done = SubUserAction.query.filter_by(
        subuser_id=sub.id,
        machine_id=machine.id,
        action_type="lube",
